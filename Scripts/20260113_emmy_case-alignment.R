...
path_case <- c(
  "Data/weekly_report_20251103-20251109.csv",
  "Data/weekly_report_20251201-20251207.csv"
)
...
df_confirmation <- df_confirmation %>%
  ...
  mutate(n = n(), row = row_number()) %>%
  ungroup() %>%
  mutate(
    confirmation_code = ifelse(
      n == 1,
...
df_cases <- df_cases %>%
  mutate(
    across(ends_with("_at"), \(x) ymd_hms(x)),
    case_number = ifelse(is.na(case_number), "[none]", case_number)
  ) %>%
  rename_with(~ paste0("case_", .x), .cols = ends_with("_at"))
...
df_cases <- df_cases %>%
  ...
  ungroup() %>%
  mutate(
    case_number = case_when(
      n == 1 ~ str_glue("{case_number}"),
...
df_aligned <- df_aligned %>%
  ...
  separate_wider_delim(
    case_number,
    delim = "_",
    names = c("case_number", "case_number_instance"),
    too_few = "align_start"
  ) %>%
  separate_wider_delim(
    confirmation_code,
    delim = "_",
    names = c("confirmation_code", "confirmation_code_instance"),
    too_few = "align_start"
  )
...
df_aligned %>%
  ...
  filter(
    date(case_completed_at) %in%
      unique(date(df_cases$case_completed_at)) |
      date(create_dt_utc) %in% unique(date(df_cases$case_completed_at))
  ) %>%
